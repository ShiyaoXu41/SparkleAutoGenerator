# ================== 2. generate vmod ==================
# ======================================================
from multi_fan_network_func import *

target_path = root_path + "\\vmod\\"

f = open(target_path + "multi_fan_network.v", mode = 'w')
...
DATA_TYPE = NUM_PEGS * OUT_DATA_TYPE # true for reduction network (reduce with FP32)

f.write("\nmodule multi_fan_network # (\n")
...
f.write("\tinput [NUM_PEGS*(NUM_PEGS*DATA_TYPE)-1 : 0] i_data_bus; // input data bus\n\n")

f.write("\tinput [(NUM_PEGS-1)-1 : 0] i_add_en_bus; // adder enable bus\n")
f.write("\tinput [3*(NUM_PEGS-1)-1 : 0] i_cmd_bus; // command bits for each adder\n")
NUM_SEL_BITS = get_sel_bits()
f.write("\tinput [" +  str(int(NUM_SEL_BITS- 1)) + " : 0] i_sel_bus; // select bits for FAN topolgy\n\n")

f.write("\toutput reg [NUM_PEGS-1 : 0] o_valid; // output valid signal\n")
f.write("\toutput reg [NUM_PEGS*(NUM_PEGS*DATA_TYPE)-1 : 0] o_data_bus; // output data bus\n\n")
...
for i in range(LOG2_PEGS - 2): # reg data type for FF
	for j in range(LOG2_PEGS - 2 - i):
		lvl_src = i
		lvl_dest = LOG2_PEGS-1-j
...
f.write("\treg [" + str(NUM_PEGS * LOG2_PEGS * DATA_TYPE - 1) + " : 0] r_lvl_output_ff;\n")
f.write("\treg [" + str(NUM_PEGS * LOG2_PEGS -1) + " : 0] r_lvl_output_ff_valid;\n") 
f.write("\n\n")


# valid FFs for each level of the adder tree
f.write("\t// valid FFs for each level of the adder tree\n")
f.write("\treg [" + str(LOG2_PEGS + 1) + " : 0] r_valid;\n") # system delay

# flop final adder output cmd and values for timing alignment
f.write("\t// flop final adder output cmd and values\n")
f.write("\treg [NUM_PEGS*DATA_TYPE-1:0] r_final_sum;\n")
...
for i in range(LOG2_PEGS - 2):
	...
	for j in range(LOG2_PEGS - 2 - i):
		...
		if (new_i == LOG2_PEGS - 2 - i - 1): # Input of fowarding link FF comes directly from adder
...
for i in range(LOG2_PEGS):
	max_range = (i+1)*NUM_PEGS*DATA_TYPE-1 
	min_range = i*NUM_PEGS*DATA_TYPE 
	max_range_v = (i+1)*NUM_PEGS-1
	min_range_v = i*NUM_PEGS
	adder_vn = []
	f.write("\talways @ (posedge clk) begin\n")
	...
	if (i == 0): # level 0
		...
		for n in range(NUM_PEGS >> (i)):
			adder_vn.append(int(2**(i)*n + 2**(i-1) - 1))
		count = 0
		for s in range(NUM_PEGS):
			if (s not in adder_vn):
				f.write("\t\t\tr_lvl_output_ff[" + str((i*DATA_TYPE*NUM_PEGS)+(s+1)*DATA_TYPE-1) + ":" + str((i*DATA_TYPE*NUM_PEGS)+s*DATA_TYPE) + "] <= r_lvl_output_ff[" + str(((i-1)*DATA_TYPE*NUM_PEGS)+(s+1)*DATA_TYPE-1) + ":" + str(((i-1)*DATA_TYPE*NUM_PEGS)+s*DATA_TYPE) + "];\n")
				f.write("\t\t\tr_lvl_output_ff_valid[" + str((i*NUM_PEGS)+s) + "] <= r_lvl_output_ff_valid[" + str((i-1)*NUM_PEGS+s) + "];\n")
			else:
				f.write("\t\t\tif (w_vn_lvl_" + str(i) + "_valid[" + str(count) + "] == 1'b1) begin\n")
				f.write("\t\t\t\tr_lvl_output_ff[" + str((i*DATA_TYPE*NUM_PEGS)+(s+1)*DATA_TYPE-1) + ":" + str((i*DATA_TYPE*NUM_PEGS)+s*DATA_TYPE) + "] <= w_vn_lvl_" + str(i) + "[" + str(DATA_TYPE * (count+1) -1) + ":" + str(DATA_TYPE * count) + "];\n")
				f.write("\t\t\t\tr_lvl_output_ff_valid[" + str((i*NUM_PEGS)+s) + "] <= 1'b1;\n")
				f.write("\t\t\tend else begin\n")
				f.write("\t\t\t\tr_lvl_output_ff[" + str((i*DATA_TYPE*NUM_PEGS)+(s+1)*DATA_TYPE-1) + ":" + str((i*DATA_TYPE*NUM_PEGS)+s*DATA_TYPE) + "] <= r_lvl_output_ff[" + str(((i-1)*DATA_TYPE*NUM_PEGS)+(s+1)*DATA_TYPE-1) + ":" + str(((i-1)*DATA_TYPE*NUM_PEGS)+s*DATA_TYPE) + "];\n")
				f.write("\t\t\t\tr_lvl_output_ff_valid[" + str((i*NUM_PEGS)+s) + "] <= r_lvl_output_ff_valid[" + str((i-1)*NUM_PEGS+s) + "];\n")
				f.write("\t\t\tend\n")
...
f.write("\tgenvar i;\n")
f.write("\tgenerate\n")
f.write("\t\tfor (i=0; i < " + str(LOG2_PEGS + 1) + "; i=i+1) begin\n")
f.write("\t\t\talways @ (posedge clk) begin\n")
f.write("\t\t\t\tif (rst == 1'b1) begin\n")
...
for adderID in range(NUM_PEGS - 1):
	...
	if (SEL_IN == 0) : # Switch needs to be hardcoded
		f.write("\t\t.SEL_IN( 2 )) my_reduct_adder_" + str(adderID) + " (\n")
	else:
		f.write("\t\t.SEL_IN( " + str(SEL_IN) + " )) my_reduct_adder_" + str(adderID) + " (\n")	
...
f.write("\t\tend else begin\n")
f.write("\t\t\tr_final_add <= i_add_en_bus[" + str((NUM_PEGS-1)-1) + "];\n")
f.write("\t\t\tr_final_add2 <= r_final_add;\n")
f.write("\t\t\tr_final_sum <= w_fan_lvl_" + str(LOG2_PEGS-1) + ";\n")
f.write("\t\t\tend\n")
f.write("\tend\n")
f.write("\n\n")
...
f.write("\t\tend else begin\n")
f.write("\t\t\to_data_bus[" + str(int(NUM_PEGS/2*DATA_TYPE-DATA_TYPE-1)) + ":0] <= r_lvl_output_ff[" + str(int(NUM_PEGS*DATA_TYPE*(LOG2_PEGS-1)+NUM_PEGS/2*DATA_TYPE-DATA_TYPE-1)) + ":" + str(int(NUM_PEGS*DATA_TYPE*(LOG2_PEGS-1))) + "];\n")
f.write("\t\t\tif (r_final_add2 == 1'b1) begin\n") # adding
f.write("\t\t\t\to_data_bus[" + str(int(NUM_PEGS/2*DATA_TYPE-1)) + ":" + str(int(NUM_PEGS/2*DATA_TYPE-DATA_TYPE)) + "] <= r_final_sum;\n")
f.write("\t\t\tend else begin\n")
f.write("\t\t\t\to_data_bus[" + str(int(NUM_PEGS/2*DATA_TYPE-1)) + ":" + str(int(NUM_PEGS/2*DATA_TYPE-DATA_TYPE)) + "] <= r_lvl_output_ff[" + str(int(NUM_PEGS*DATA_TYPE*(LOG2_PEGS-1)+NUM_PEGS/2*DATA_TYPE-1)) + ":" + str(int(NUM_PEGS*DATA_TYPE*(LOG2_PEGS-1)+NUM_PEGS/2*DATA_TYPE-DATA_TYPE)) + "];\n")
f.write("\t\t\tend\n")
f.write("\t\t\to_data_bus[" + str(int(NUM_PEGS*DATA_TYPE-1)) + ":" + str(int(NUM_PEGS/2*DATA_TYPE)) + "] <= r_lvl_output_ff[" + str(int(NUM_PEGS*DATA_TYPE*LOG2_PEGS-1)) + ":" + str(int(NUM_PEGS*DATA_TYPE*(LOG2_PEGS-1)+NUM_PEGS/2*DATA_TYPE)) + "];\n") 
f.write("\t\tend\n")
f.write("\tend\n")
f.write("\n\n")
...
f.write("\talways @ (*) begin\n")
f.write("\t\tif (rst == 1'b1 || r_valid[" + str(LOG2_PEGS + 1) + "] == 1'b0) begin\n")
f.write("\t\t\to_valid <= 'd0;\n")
f.write("\t\tend else begin\n")
f.write("\t\t\to_valid[" + str(int(NUM_PEGS/2-2)) + ":0] <= r_lvl_output_ff_valid[" + str(int(NUM_PEGS*(LOG2_PEGS-1)+NUM_PEGS/2-1)) + ":" + str(int(NUM_PEGS*(LOG2_PEGS-1))) + "];\n")
f.write("\t\t\tif (r_final_add2 == 1'b1) begin\n") # adding
f.write("\t\t\t\to_valid[" + str(int(NUM_PEGS/2-1)) + "] <= 1'b1 ;\n")
f.write("\t\t\tend else begin\n")
f.write("\t\t\t\to_valid[" + str(int(NUM_PEGS/2-1)) + "] <= r_lvl_output_ff_valid[" + str(int(NUM_PEGS*(LOG2_PEGS-1)+NUM_PEGS/2-1)) + "];\n")
f.write("\t\t\tend\n")
f.write("\t\t\to_valid[" + str(int(NUM_PEGS-1)) + ":" + str(int(NUM_PEGS/2)) + "] <= r_lvl_output_ff_valid[" + str(int(NUM_PEGS*LOG2_PEGS-1)) + ":" + str(int(NUM_PEGS*(LOG2_PEGS-1)+NUM_PEGS/2)) + "];\n") 
f.write("\t\tend\n")
f.write("\tend\n")
f.write("\n\n")
