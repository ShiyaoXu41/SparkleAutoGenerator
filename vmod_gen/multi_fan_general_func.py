...
def num_adders_in_lvl(i):
	return int(NUM_PEGS >> (i+1)) # level starts at 0
	
# determine if adder is an edge or regular adder
def is_edge(adderID):
	lvl = get_lvl(adderID)
	if (((NUM_PEGS-1)-adderID < 2**(lvl+1)) or (adderID < 2**(lvl+1))):
		return "true"
	else:
		return "false"
...
def get_adder_en_id(adderID):
	...
	num_adders = 0
	for i in range(lvl):
		num_adders = num_adders + (NUM_PEGS >> (i+1))
...
def get_lvl_cmd_range(lvl):
	...
	end_adder = 0
	for i in range(lvl):
		start_adder = start_adder + (NUM_PEGS >> (i+1))
	for i in range(lvl+1):
		end_adder = end_adder + (NUM_PEGS >> (i+1))
