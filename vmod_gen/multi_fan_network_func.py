...
DATA_TYPE = OUT_DATA_TYPE * NUM_PEGS # (FAN topology uses FP32 reduction)

# get number of binary tree wires per level
def get_binary_fwd_wires(lvl):
	if ( (NUM_PEGS >> (lvl+1))  > 2):
		num_tree_wire = ( (NUM_PEGS >> (lvl+1)) - 2)*2 + 2 # due to 2 edge adders
	else:
		num_tree_wire = ((NUM_PEGS >> (lvl+1)))
...
def get_num_fwd_links(lvl_src, lvl_dest):
	...
	for s in range(lvl_dest - lvl_src - 2): # final dest lvl - src lvl
		sub = sub + (NUM_PEGS >> (s+2+lvl_src));  # sub depends on num_PEGS, s, and lvl_src 
	num_links = ((NUM_PEGS >> (lvl_src+1))-2) - int(sub) 
	return num_links
...
def get_ff_fwd_index(lvl_src, lvl_dest, n):
	num_wire = 4*((NUM_PEGS / 2**lvl_dest)-1) + 2 # find # of wires from the previous fan ff level
...
def get_fan_out(adderID):
	lvl = get_lvl(adderID)
	shift = get_adder_lvl_shift(adderID)
	if (lvl == LOG2_PEGS -1): # last level adder logic
		return "w_fan_lvl_" + str(lvl) + "[" + str(DATA_TYPE  -1 )+ " : " + str(0)  + "]"
	elif ((NUM_PEGS-1)-adderID < 2**(lvl+1)): # last edge adder of lvl
