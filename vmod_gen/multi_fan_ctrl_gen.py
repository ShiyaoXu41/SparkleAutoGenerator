# ================== 2. generate vmod ==================
# ======================================================
from multi_fan_ctrl_func import *

target_path = root_path + "\\vmod\\"

f = open(target_path + "multi_fan_ctrl.v", mode = 'w')
...
f.write("\tinput clk;\n")
f.write("\tinput rst;\n")
f.write("\tinput [NUM_PEGS*LOG2_PEGS-1: 0] i_vn; // different partial sum bit seperator\n")
...
f.write("\toutput reg [(NUM_PEGS-1)-1:0] o_reduction_add; // determine to add or not\n")
f.write("\toutput reg [3*(NUM_PEGS-1)-1:0] o_reduction_cmd; // reduction command (for VN commands)\n")
...
f.write("\treg [(NUM_PEGS-1)-1:0] r_reduction_add;\n")
f.write("\treg [3*(NUM_PEGS-1)-1:0] r_reduction_cmd;\n")
...
f.write("\treg [" + str(CMD_SEL_DELAY) + "*NUM_PEGS*LOG2_PEGS-1:0] r_vn;\n")
f.write("\treg [NUM_PEGS*LOG2_PEGS-1:0] w_vn;\n")
...
f.write("\t\t\t\t\t\tr_vn[(i+1)*NUM_PEGS*LOG2_PEGS-1:i*NUM_PEGS*LOG2_PEGS] <= 'd0;\n")
f.write("\t\t\t\t\tend else begin\n")
f.write("\t\t\t\t\t\tr_vn[(i+1)*NUM_PEGS*LOG2_PEGS-1:i*NUM_PEGS*LOG2_PEGS] <= i_vn;\n")
f.write("\t\t\t\t\tend\n")
f.write("\t\t\t\tend\n")
...
f.write("\t\t\t\t\t\tr_vn[(i+1)*NUM_PEGS*LOG2_PEGS-1:i*NUM_PEGS*LOG2_PEGS] <= 'd0;\n")
f.write("\t\t\t\t\tend else begin\n")
f.write("\t\t\t\t\t\tr_vn[(i+1)*NUM_PEGS*LOG2_PEGS-1:i*NUM_PEGS*LOG2_PEGS] <= r_vn[i*NUM_PEGS*LOG2_PEGS-1:(i-1)*NUM_PEGS*LOG2_PEGS];\n")
f.write("\t\t\t\t\tend\n")
f.write("\t\t\t\tend\n")
f.write("\t\t\tend\n")
f.write("\t\tend\n")
...
f.write("\t\tw_vn = r_vn[" + str(CMD_SEL_DELAY) + "*NUM_PEGS*LOG2_PEGS-1:" + str(CMD_SEL_DELAY-1) + "*NUM_PEGS*LOG2_PEGS];\n")
f.write("\tend\n")
f.write("\n\n")
...
for i in range(LOG2_PEGS):
	...
	if ( i < LOG2_PEGS -1): 
		...
		f.write("\tgenerate\n")
		f.write("\t\tfor (x=0; x < " + str(NUM_PEGS >> (i+1)) + "; x=x+1) begin: adders_lvl_" + str(i) + "\n")
		############################################### LEFT CASE ###########################################
		...
		f.write("\t\t\t\tend\n")
		############################################### RIGHT CASE ###########################################
		f.write("\t\t\tend else if (x == " + str((NUM_PEGS >> (i+1)) -1 )  + ") begin: r_edge_case\n")
		f.write("\t\t\t\talways @ (posedge clk) begin\n")
		f.write("\t\t\t\t\tif (rst == 1'b1) begin\n")
		f.write("\t\t\t\t\t\tr_reduction_add[" + get_cmd_shift_accum(i) +  "+x] <= 'd0;\n")
		f.write("\t\t\t\t\t\tr_reduction_cmd[3*" + get_cmd_shift_accum(i) +  "+3*x+:3] <= 'd0;\n")
...
f.write("\t\tend else begin\n")
f.write("\t\t\to_reduction_add <= " + gen_o_reduction_add(LOG2_PEGS, NUM_PEGS) + "\n")
f.write("\t\t\to_reduction_cmd <= " + gen_o_reduction_cmd(LOG2_PEGS, NUM_PEGS) + "\n")
f.write("\t\t\to_reduction_sel <= " + gen_o_reduction_sel(LOG2_PEGS, NUM_PEGS) + "\n")
f.write("\t\tend\n")
f.write("\tend\n\n")
